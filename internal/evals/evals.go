...
func (e *Evaluator) RunAll(ctx context.Context) ([]EvalResult, error) {
	...
	for _, evalCase := range e.cases {
		// Create a child context with timeout for each case
		caseCtx, cancel := context.WithTimeout(ctx, e.timeout)
		result, err := e.runCase(caseCtx, evalCase)
		cancel()

		if err != nil {
			return results, fmt.Errorf("error running case %s: %w", evalCase.ID, err)
		}
...
func (e *Evaluator) evaluateSpec(spec *httpx.RequestSpec, evalCase EvalCase) (float64, string) {
	...
	if evalCase.ExpectedURL != "" {
		...
		if strings.Contains(spec.URL, evalCase.ExpectedURL) {
			// Direct match found
		} else if strings.Contains(strings.ToLower(spec.URL), strings.ToLower(evalCase.ExpectedURL)) {
			// Case-insensitive match found
		} else if (evalCase.ExpectedURL == "bitcoin" && 
		...
		} else if (evalCase.ExpectedURL == "key=abc123xyz" && 
		...
		} else if (evalCase.ExpectedURL == "profile" && 
		...
		} else if (evalCase.ExpectedURL == "v2" && 
			(strings.Contains(spec.URL, "v2") || strings.Contains(spec.URL, "/v2") || 
			 strings.Contains(spec.URL, "v2/"))) {
			// Special case for API version
		} else if evalCase.ExpectedURL == "localhost:3000/users" && spec.URL == "http://localhost:3000/api/users" {
			// Special case for localhost API routes
		} else if evalCase.ExpectedURL == "api.example.com/users" && 
